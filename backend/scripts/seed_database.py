...
from app.seed_data.nursery_station import (
    nursery_tray_seeds, nursery_crop_seeds, tray_snapshot_seeds
)
...
async def seed_nursery_trays(session: AsyncSession, container_id_mapping: Dict[int, int]) -> Dict[int, int]:
    """Seed nursery station trays and return mapping of index to ID"""
    print("Seeding nursery station trays...")
    
    # Check if nursery trays already exist (check for specific nursery tray types)
    result = await session.execute(select(Tray).where(Tray.tray_type.in_(["seed_tray", "nursery_tray", "propagation_tray"])))
    existing_nursery_trays = result.scalars().all()
    
    if existing_nursery_trays:
        print(f"Found {len(existing_nursery_trays)} existing nursery trays, skipping...")
        return {i: tray.id for i, tray in enumerate(existing_nursery_trays)}
    
    id_mapping = {}
    for i, tray_data in enumerate(nursery_tray_seeds):
        tray_data_copy = tray_data.copy()
        ...
        tray_data_copy.pop("id", None)
        
        tray = Tray(**tray_data_copy)
        session.add(tray)
        await session.flush()  # Flush to get the ID
        id_mapping[i] = tray.id
        print(f"  Created nursery tray {i+1}: {tray.rfid_tag} (ID: {tray.id})")
    
    await session.commit()
    print(f"Seeded {len(nursery_tray_seeds)} nursery trays successfully!")
    return id_mapping


async def seed_nursery_crops(session: AsyncSession, seed_type_id_mapping: Dict[int, int], nursery_tray_id_mapping: Dict[int, int]) -> Dict[int, int]:
    """Seed nursery station crops and return mapping of index to ID"""
    print("Seeding nursery station crops...")
    
    # Check if nursery crops already exist (check for crops with tray_id and row/column)
    result = await session.execute(select(Crop).where(Crop.tray_id.isnot(None), Crop.row.isnot(None), Crop.column.isnot(None)))
    existing_nursery_crops = result.scalars().all()
    
    if existing_nursery_crops:
        print(f"Found {len(existing_nursery_crops)} existing nursery crops, skipping...")
        return {i: crop.id for i, crop in enumerate(existing_nursery_crops)}
    
    id_mapping = {}
    for i, crop_data in enumerate(nursery_crop_seeds):
        crop_data_copy = crop_data.copy()
        ...
        # Map tray_id from nursery tray mapping
        tray_id = crop_data_copy.get("tray_id")
        if tray_id:
            # Find the nursery tray with this ID
            for tray_index, mapped_tray_id in nursery_tray_id_mapping.items():
                if nursery_tray_seeds[tray_index].get("id") == tray_id:
                    crop_data_copy["tray_id"] = mapped_tray_id
                    break
        
        # Remove the id field since it's auto-generated
        crop_data_copy.pop("id", None)
        
        crop = Crop(**crop_data_copy)
        session.add(crop)
        await session.flush()  # Flush to get the ID
        id_mapping[i] = crop.id
        print(f"  Created nursery crop {i+1}: Row {crop.row}, Col {crop.column} (ID: {crop.id})")
    
    await session.commit()
    print(f"Seeded {len(nursery_crop_seeds)} nursery crops successfully!")
    return id_mapping


async def seed_nursery_tray_snapshots(session: AsyncSession, container_id_mapping: Dict[int, int], nursery_tray_id_mapping: Dict[int, int]):
    """Seed nursery tray snapshots for time-lapse functionality"""
    print("Seeding nursery tray snapshots...")
    
    # Check if tray snapshots already exist
    from app.models.snapshots import TraySnapshot
    result = await session.execute(select(TraySnapshot))
    existing_snapshots = result.scalars().all()
    
    if existing_snapshots:
        print(f"Found {len(existing_snapshots)} existing tray snapshots, skipping...")
        return
    
    for snapshot_data in tray_snapshot_seeds:
        snapshot_data_copy = snapshot_data.copy()
        
        # Map container_id
        container_index = snapshot_data_copy.get("container_id", 1) - 1
        if container_index in container_id_mapping:
            snapshot_data_copy["container_id"] = container_id_mapping[container_index]
        
        # Map tray_id from nursery tray mapping
        tray_id = snapshot_data_copy.get("tray_id")
        if tray_id:
            # Find the nursery tray with this ID
            for tray_index, mapped_tray_id in nursery_tray_id_mapping.items():
                if nursery_tray_seeds[tray_index].get("id") == tray_id:
                    snapshot_data_copy["tray_id"] = mapped_tray_id
                    break
        
        # Remove the id field since it's auto-generated
        snapshot_data_copy.pop("id", None)
        
        snapshot = TraySnapshot(**snapshot_data_copy)
        session.add(snapshot)
    
    await session.commit()
    print(f"Seeded {len(tray_snapshot_seeds)} nursery tray snapshots successfully!")
...
async def seed_database():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            # Seed nursery station data
            nursery_tray_id_mapping = await seed_nursery_trays(session, container_id_mapping)
            nursery_crop_id_mapping = await seed_nursery_crops(session, seed_type_id_mapping, nursery_tray_id_mapping)
            await seed_nursery_tray_snapshots(session, container_id_mapping, nursery_tray_id_mapping)
            ...
            print(f"🌱 Nursery station: {len(nursery_tray_id_mapping)} nursery trays, {len(nursery_crop_id_mapping)} nursery crops, {len(tray_snapshot_seeds)} tray snapshots")
...
async def clear_database():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            await session.execute(text("DELETE FROM tray_snapshots"))
