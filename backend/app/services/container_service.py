...
class ContainerService:
        ...
        return ContainerPublic(
            ...
            system_integrations=SystemIntegrations(
                fa_integration=SystemIntegration(
                    **container_dict["system_integrations"]["fa_integration"]
                ),
                aws_environment=SystemIntegration(
                    **container_dict["system_integrations"]["aws_environment"]
                ),
                mbai_environment=SystemIntegration(
                    **container_dict["system_integrations"]["mbai_environment"]
                ),
            ),
    ...
    def get_containers(self, skip: int = 0, limit: int = 100) -> ContainersPublic:
        ...
        return ContainersPublic(data=container_publics, count=len(container_publics))
    ...
    def get_containers_filtered(
        ...
        containers, total = self.repository.get_containers_filtered(
            skip=skip, limit=limit, filters=filters or {}
        )
        ...
        for container in containers:
            ...
            container_summary = ContainerSummary(
                ...
                name=container_dict["name"],
                type=container_dict["type"],
                tenant_name=container_dict[
                    "tenant"
                ],  # Using tenant as tenant_name for now
                purpose=container_dict["purpose"],
        ...
        return ContainerList(total=total, results=container_summaries)
    ...
    def create_container_from_form(
        ...
        location = ContainerLocation(
            city=location_city, country=location_country, address=form_data.location
        )
        ...
        system_integrations = SystemIntegrations(
            fa_integration=SystemIntegration(
                name="Alpha"
                if form_data.purpose == ContainerPurpose.PRODUCTION
                else "Dev",
                enabled=form_data.connect_to_other_systems,
            ),
            aws_environment=SystemIntegration(
                name="Prod"
                if form_data.purpose == ContainerPurpose.PRODUCTION
            ...
            ),  # Disabled by default as per spec
        )
        ...
        if not metrics:
            ...
            public_metrics = ContainerMetricsPublic(
                ...
                nursery_utilization=MetricValue(
                    **default_metrics_dict["nursery_utilization"]
                ),
                cultivation_utilization=MetricValue(
                    **default_metrics_dict["cultivation_utilization"]
                ),
            )
        else:
            ...
            public_metrics = ContainerMetricsPublic(
                ...
                cultivation_utilization=MetricValue(
                    **metrics_dict["cultivation_utilization"]
                ),
            )
    ...
    def get_container_crops(
        ...
        return ContainerCropsPublic(total=total, results=crop_publics)
    ...
    def get_container_metric_cards(self, container_id: str) -> list[dict]:
        ...
        if not metrics:
            raise HTTPException(
                status_code=404, detail="Metrics not found for this container"
            )
        ...
        metric_cards = [
            {
                ...
                "targetValue": metrics_dict["temperature"].get(
                    "target_display",
                    f"{metrics_dict['temperature'].get('target', '')}°C",
                ),
            ...
            },
            {
                ...
                "targetValue": metrics_dict["humidity"].get(
                    "target_display", f"{metrics_dict['humidity'].get('target', '')}%"
                ),
                "icon": "WaterDrop",
            },
            {
                ...
                "targetValue": metrics_dict["co2"].get(
                    "target_display", f"{metrics_dict['co2'].get('target', '')}ppm"
                ),
                "icon": "Co2",
            },
            {
                ...
                "targetValue": metrics_dict["yield_metric"].get(
                    "target_display",
                    f"+{metrics_dict['yield_metric'].get('trend', 0)}Kg",
                ),
                "icon": "Agriculture",
            },
            {
                ...
                "targetValue": metrics_dict["nursery_utilization"].get(
                    "target_display",
                    f"+{metrics_dict['nursery_utilization'].get('trend', 0)}%",
                ),
            ...
            },
            {
                ...
                "targetValue": metrics_dict["cultivation_utilization"].get(
                    "target_display",
                    f"+{metrics_dict['cultivation_utilization'].get('trend', 0)}%",
                ),
                "icon": "ViewWeek",
            },
        ]
    ...
    def shutdown_container(self, container_id: str) -> ContainerPublic:
        ...
        from app.models import ContainerStatus, ContainerUpdate
