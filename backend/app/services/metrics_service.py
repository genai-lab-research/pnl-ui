...
class MetricsService:
    ...
    def get_container_metrics(
        ...
        metrics = self.container_repository.get_container_metrics(container_id, time_range)
        if not metrics:
        ...
        return {
            "yield_data": [
                {"date": "2025-05-01", "value": metrics_dict['yield_metric']['current']},
                {"date": "2025-05-02", "value": metrics_dict['yield_metric']['current'] + 5},
                {"date": "2025-05-03", "value": metrics_dict['yield_metric']['current'] - 3},
                {"date": "2025-05-04", "value": metrics_dict['yield_metric']['current'] + 2},
                {"date": "2025-05-05", "value": metrics_dict['yield_metric']['current'] + 1},
            ],
            "space_utilization_data": [
                {"date": "2025-05-01", "value": metrics_dict['nursery_utilization']['current']},
                {"date": "2025-05-02", "value": metrics_dict['nursery_utilization']['current'] + 2},
                {"date": "2025-05-03", "value": metrics_dict['nursery_utilization']['current'] - 1},
                {"date": "2025-05-04", "value": metrics_dict['nursery_utilization']['current'] + 3},
                {"date": "2025-05-05", "value": metrics_dict['nursery_utilization']['current']},
            ],
            "average_yield": metrics_dict['yield_metric']['current'],
            "total_yield": metrics_dict['yield_metric']['current'] * 7,  # Mock weekly total
    ...
    def get_performance_overview(self, time_range: Optional[TimeRange] = TimeRange.WEEK) -> Any:
        ...
        if virtual_containers:
            virtual_metrics = self.container_repository.get_container_metrics(
                virtual_containers[0].id if virtual_containers else physical_containers[0].id,
                time_range
            )
        ...
        if not physical_metrics and physical_containers:
            ...
            physical_base_yield = physical_metrics.yield_metric['current'] if physical_metrics else 63
            physical_base_utilization = physical_metrics.nursery_utilization['current'] if physical_metrics else 80
            
        if not virtual_metrics and virtual_containers:
            virtual_base_yield = 55
            virtual_base_utilization = 65
        else:
            virtual_base_yield = virtual_metrics.yield_metric['current'] if virtual_metrics else 55
            virtual_base_utilization = virtual_metrics.nursery_utilization['current'] if virtual_metrics else 65
        ...
        return {
            "physical": {
                ...
                "yield": {
                    ...
                    "totalYield": int(physical_base_yield * 7 / 5)  # Weekly total approximation
                },
                "spaceUtilization": {
                    "labels": ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
            ...
            "virtual": {
                ...
                "yield": {
                    ...
                    "totalYield": int(virtual_base_yield * 7 / 5)  # Weekly total approximation
                },
                "spaceUtilization": {
                    "labels": ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
