...
class TestMetricsAPI:
    ...
    def test_get_container_metrics(self):
        ...
        if containers:
            ...
            required_fields = [
                "yield_data", "space_utilization_data", "average_yield",
                "total_yield", "average_space_utilization", "current_temperature",
                "current_humidity", "current_co2", "crop_counts", "is_daily"
            ]
    ...
    def test_get_container_metrics_with_time_range(self):
        ...
        if containers:
            ...
            for time_range in [TimeRange.WEEK, TimeRange.MONTH, TimeRange.QUARTER, TimeRange.YEAR]:
                response = client.get(f"/api/v1/metrics/container/{container_id}?time_range={time_range}")
                assert response.status_code == 200
                data = response.json()
    ...
    def test_get_nonexistent_container_metrics(self):
        ...
        assert response.status_code == 200  # Returns empty data for non-existent containers
        data = response.json()
    ...
    def test_get_performance_overview_with_time_range(self):
        ...
        for time_range in [TimeRange.WEEK, TimeRange.MONTH, TimeRange.QUARTER, TimeRange.YEAR]:
            response = client.get(f"/api/v1/metrics/performance?time_range={time_range}")
            assert response.status_code == 200
            data = response.json()
