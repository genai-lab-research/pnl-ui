...
class ContainerRepository(BaseRepository[Container, ContainerCreate, ContainerUpdate]):
    ...
    async def get_with_relationships_detached(self, container_id: int) -> Optional[Container]:
        ...
        if container:
            # Don't force load relationships - selectinload should handle this
            # Only expunge the object to detach it from the session
    ...
    async def get_filtered(
        ...
        # Always join with tenant table to ensure consistent query structure
        query = query.select_from(Container).outerjoin(Tenant, Container.tenant_id == Tenant.id)
        
        if filters.search:
            logger.debug(f"Search query with tenant join for term: {filters.search}")
        ...
        if load_relationships:
            query = query.options(
                ...
                selectinload(Container.alerts),
                selectinload(Container.tenant),
                # Don't load metric_snapshots by default - they're large
                # selectinload(Container.metric_snapshots)
            )
        ...
        if filters.search:
            ...
            # Create search conditions that handle NULL tenant names properly
            container_conditions = or_(
                Container.name.ilike(search_term),
                Container.notes.ilike(search_term)
            )
            # Search in tenant name - the outer join handles NULL tenants
            tenant_condition = Tenant.name.ilike(search_term)
            
            search_condition = or_(container_conditions, tenant_condition)
            conditions.append(search_condition)
            logger.debug(f"Applied search conditions for: {search_term}")
        ...
        # Always join with tenant table for count query to match main query
        count_query = count_query.select_from(Container).outerjoin(Tenant, Container.tenant_id == Tenant.id)
        
        if conditions:
            count_query = count_query.where(and_(*conditions))
        
        total_count = await self.db.scalar(count_query) or 0
        logger.debug(f"Total count for search: {total_count}")
        ...
        logger.debug(f"Found {len(containers)} containers for search")
        
        # Don't force load relationships - let them be lazy loaded
        # Only expunge if not loading relationships to avoid detached instance issues
    ...
    async def get_metrics_data(
        ...
        """Get containers for metrics calculation with optimized query."""
        from sqlalchemy.orm import selectinload
        
        # Use a more efficient query that only loads necessary data
    ...
    async def get_filter_options(self) -> Dict[str, Any]:
        """Get available filter options with sequential queries."""
        from app.models.tenant import Tenant
        
        # Execute queries sequentially to avoid session state issues
