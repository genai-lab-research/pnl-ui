...
class ContainerRepository:
    ...
    def _initialize_mock_data(self):
        ...
        metrics = ContainerMetrics(
            ...
            temperature={"current": 20, "unit": "°C", "target": 21, "target_display": "21°C"},
            humidity={"current": 65, "unit": "%", "target": 68, "target_display": "68%"},
            co2={"current": 860, "unit": "ppm", "target": 900, "target_display": "800-900ppm"},
            yield_metric={"current": 51, "unit": "KG", "trend": 1.5, "target_display": "+1.5Kg"},
            nursery_utilization={"current": 75, "unit": "%", "trend": 5, "target_display": "+5%"},
            cultivation_utilization={"current": 90, "unit": "%", "trend": 15, "target_display": "+15%"}
        )
    ...
    def create_container(self, container_data: ContainerCreate) -> Container:
        ...
        if container_dict.get("seed_types"):
            for i, seed_type in enumerate(container_dict["seed_types"][:4]):  # Limit to 4 crops
        ...
        from app.models import ContainerActivity, ActivityType, ActivityUser
    ...
    def update_container(self, container_id: str, container_data: ContainerUpdate) -> Optional[Container]:
    ...
    # Metrics operations
    def get_container_metrics(self, container_id: str, time_range: TimeRange = TimeRange.WEEK) -> Optional[ContainerMetrics]:
    ...
    def create_container_metrics(self, metrics_data: ContainerMetricsCreate) -> ContainerMetrics:
    ...
    def get_container_activities(self, container_id: str, limit: int = 5) -> list[ContainerActivity]:
    ...
    def create_container_activity(self, activity_data: ContainerActivityCreate) -> ContainerActivity:
    ...
    def get_containers_filtered(
        ...
        if filters:
            # Apply filters
            if filters.get("name"):
                containers = [c for c in containers if filters["name"].lower() in c.name.lower()]
            ...
            if filters.get("purpose"):
                containers = [c for c in containers if c.purpose.value == filters["purpose"]]
            ...
            if filters.get("location"):
                ...
                containers = [
                    ...
                    if (location_filter in c.location.get("city", "").lower() or 
                        location_filter in c.location.get("country", "").lower())
                ]
            ...
            if filters.get("has_alerts") is not None:
                ...
                if filters["has_alerts"]:
                    ...
                    containers = containers[:len(containers)//2] if containers else []
