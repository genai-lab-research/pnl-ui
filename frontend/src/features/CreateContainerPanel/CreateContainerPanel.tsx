...
import { ContainerCreateRequest } from '../../types/container';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createRequest: ContainerCreateRequest = {
        ...
        seed_type_ids: formData.seed_types.map(id => Number(id)),
        notes: formData.notes || undefined,
        shadow_service_enabled: formData.shadow_service_enabled,
        copied_environment_from: formData.copied_environment_from ? Number(formData.copied_environment_from) : undefined,
        robotics_simulation_enabled: formData.robotics_simulation_enabled,
        ecosystem_connected: formData.ecosystem_connected
      };

      const response = await containerApiService.createContainer(createRequest);
      
      if (onSuccess && response) {
        // Find the tenant name for the response
        const selectedTenant = tenants.find(t => t.id === Number(formData.tenant_id));
        
        // Convert API response to shared format
