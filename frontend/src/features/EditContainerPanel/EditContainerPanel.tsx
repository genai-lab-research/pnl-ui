...
import { UpdateContainerRequest, Container } from '../../shared/types/containers';
...
export const EditContainerPanel: React.FC<EditContainerPanelProps> = ({
  ...
  const loadData = async () => {
    try {
      ...
      setSeedTypes([
        { id: 1, name: 'Someroots', variety: 'V1' },
        { id: 2, name: 'sunflower', variety: 'Standard' }
      ] as any);
      setContainers([]);
    }
  };
  ...
  const populateFormData = (containerData: Container) => {
    ...
    setFormData({
      ...
      seed_types: containerData.seed_types?.map(st => parseInt(st)) || [],
  ...
  const handleFormChange = (field: keyof EditContainerFormData, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
