...
// Import services
import containerService, { ContainerStats, ContainerFilterParams, ContainerList } from '../../services/containerService';
...
export const ContainerManagementDashboard: React.FC<ContainerManagementDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    loadContainers();
  }, [searchValue, containerType, tenant, purpose, status, hasAlerts, paginationInfo.currentPage, paginationInfo.pageSize]);
        ...
        try {
          ...
          const physicalMetricsData: MetricResponse = {
            ...
            space_utilization_data: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => {
              const index = performanceData.physical.spaceUtilization.labels.findIndex(label => label.startsWith(day));
              return {
                date: day,
          ...
          const virtualMetricsData: MetricResponse = {
            yield_data: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => {
              const index = performanceData.virtual.yield.labels.findIndex(label => label.startsWith(day));
              return {
                date: day,
            ...
            space_utilization_data: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => {
              const index = performanceData.virtual.spaceUtilization.labels.findIndex(label => label.startsWith(day));
              return {
                date: day,
  ...
  return (
    <Box className={className} sx={{ width: '100%', bgcolor: 'background.default', minHeight: '100vh' }}>
      {/* Header */}
      <HeaderContainer 
        title="Control Panel" 
      ...
      >
        ...
        <Box sx={{ 
          ...
          <Box sx={{ width: { xs: '100%', md: '50%' } }}>
            <PerformanceCard
              ...
              yieldData={physicalMetrics?.yield_data.map(d => {
                let day;
                // Format the day label based on time range
                if (timeRange === 'week') {
                  day = d.date.substring(0, 3); // Mon, Tue, etc.
                } else if (timeRange === 'month') {
                  day = d.date.includes('Day') ? d.date.substring(4) : new Date(d.date).getDate().toString();
              ...
              utilizationData={physicalMetrics?.space_utilization_data.map(d => {
                let day;
                // Format the day label based on time range
                if (timeRange === 'week') {
                  day = d.date.substring(0, 3); // Mon, Tue, etc.
          ...
          <Box sx={{ width: { xs: '100%', md: '50%' } }}>
            <PerformanceCard
              ...
              yieldData={virtualMetrics?.yield_data.map(d => {
                let day;
                // Format the day label based on time range
                if (timeRange === 'week') {
                  day = d.date.substring(0, 3); // Mon, Tue, etc.
              ...
              utilizationData={virtualMetrics?.space_utilization_data.map(d => {
                let day;
                // Format the day label based on time range
                if (timeRange === 'week') {
                  day = d.date.substring(0, 3); // Mon, Tue, etc.
        ...
        <Box sx={{ mb: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, alignItems: 'center', ml: 1 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              <Typography variant="h6" fontWeight={500}>Containers List</Typography>
              <Button 
                variant="text" 
                color="primary" 
            ...
            <CreateContainerButton onClick={handleCreateContainer} onContainerCreated={handleContainerCreated} />
          </Box>
        ...
        {!loading.containers && !error.containers && paginationInfo.totalItems > 0 && (
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 2, mx: 1 }}>
            <Typography variant="body2" color="text.secondary">
              Showing page {paginationInfo.currentPage + 1} of {Math.ceil(paginationInfo.totalItems / paginationInfo.pageSize) || 2}
            </Typography>
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Box
                component="button"
                onClick={() => handlePageChange(paginationInfo.currentPage + 1)}
                disabled={paginationInfo.currentPage >= Math.ceil(paginationInfo.totalItems / paginationInfo.pageSize) - 1}
                sx={{
                  border: '1px solid',
                  borderColor: '#E0E0E0',
                  borderRadius: 1,
                  ...
                  py: 1,
                  bgcolor: 'background.paper',
                  color: paginationInfo.currentPage >= Math.ceil(paginationInfo.totalItems / paginationInfo.pageSize) - 1
                    ? '#BDBDBD'
                    : '#757575',
                  cursor: paginationInfo.currentPage >= Math.ceil(paginationInfo.totalItems / paginationInfo.pageSize) - 1
                    ? 'default'
                    : 'pointer',
                  display: 'flex',
                  alignItems: 'center',
                  '&:hover': {
                    bgcolor: paginationInfo.currentPage >= Math.ceil(paginationInfo.totalItems / paginationInfo.pageSize) - 1
                      ? 'initial'
                      : '#F5F5F5',
                  },
                  fontSize: '0.875rem',
