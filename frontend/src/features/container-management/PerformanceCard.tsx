...
export const PerformanceCard: React.FC<PerformanceCardProps> = ({
  ...
  return (
    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3.5, mt: 0.5 }}>
        ...
        <Box sx={{ flex: 1, mr: 2 }}>
          <Box sx={{ mb: 1 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', mb: 0.5 }}>
              <Typography variant="body2" color="text.secondary" sx={{ mr: 1, fontSize: '0.75rem', fontWeight: 400 }}>
                AVG
              </Typography>
          ...
          <Box sx={{ height: 200, width: '100%', position: 'relative' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                ...
                barSize={16}
              >
                <CartesianGrid horizontal={true} vertical={false} stroke="#E0E0E0" strokeDasharray="3 3" />
                <XAxis 
                  dataKey="day"
                  axisLine={false}
                  tickLine={false}
                  ...
                  interval={timeRange === 'year' ? 1 : (timeRange === 'month' && yieldData.length > 12) ? 2 : 0}
                />
                <YAxis hide />
                <Tooltip 
                ...
                {yieldData.length > 0 && 
                  <ReferenceLine 
                    x={yieldData.reduce((maxObj, current) => current.value > maxObj.value ? current : maxObj, yieldData[0]).day} 
                    stroke="#000" 
                    strokeDasharray="0" 
                    isFront={true}
                    strokeWidth={0}
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                {utilizationData.length > 0 && 
                  <ReferenceLine 
                    x={utilizationData.reduce((maxObj, current) => current.value > maxObj.value ? current : maxObj, utilizationData[0]).day} 
                    stroke="#000" 
                    strokeDasharray="0" 
                    isFront={true}
                    strokeWidth={0}
