...
import { EditContainerModal } from '../container-edit/components/EditContainerModal';
import {
              ...
              {isLoading.overview ? (
                ...
                >
                  ...
                  <Box sx={{ backgroundColor: '#fff' }}>
                    {cropsTableData.rows.map((row: any, index: number) => (
                      <Box
                        key={row.id || index}
                        sx={{
              ...
              <Grid item xs={12} md={4}>
                ...
                <ContainerInfoPanel
                  containerInfo={[
                    ...
                    { label: 'Location', value: containerInfo?.location ? ((containerInfo.location as any).city || (containerInfo.location as any).address || (containerInfo.location as any).name || 'N/A') : 'N/A' },
                    { label: 'Status', value: (() => {
                      if (!containerInfo?.status) return 'Active';
                      switch (containerInfo.status) {
                        case 'active': return 'Active';
                    ...
                    { label: 'Seed Type', value: containerInfo?.seed_types && containerInfo.seed_types.length > 0 ? containerInfo.seed_types.map((st: any) => st.name).join(', ') : 'N/A' },
