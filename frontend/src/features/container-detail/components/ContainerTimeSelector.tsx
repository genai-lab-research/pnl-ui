...
export const ContainerTimeSelector: React.FC<ContainerTimeSelectorProps> = ({
  ...
  const selectorOptions = timeRangeOptions.map(option => ({
    value: option.label as TimeRange, // TimeRangeSelector expects 'Week', 'Month', etc.
  ...
  const selectedValue = selectedTimeRange.charAt(0).toUpperCase() + selectedTimeRange.slice(1) as TimeRange;
