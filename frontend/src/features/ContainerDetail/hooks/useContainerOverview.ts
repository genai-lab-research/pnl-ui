...
import { ContainerOverviewViewModel, ContainerOverviewState } from '../viewmodels/ContainerOverviewViewModel';
...
export const useContainerOverview = (containerId: number): UseContainerOverviewResult => {
  ...
  const cropsTableData = viewModelRef.current?.getCropsTableData() ?? [];
