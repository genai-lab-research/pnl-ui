...
export const ContainerDataRow: React.FC<ContainerDataRowProps> = ({
  ...
  const cells: CellData[] = [
    ...
    {
      id: 'status',
      type: status === 'overdue' ? 'badge' : 'text',
      content: status === 'overdue' ? (String(batchSize).split(' ')[0] || '2') + ' days' : '0',
      statusVariant: status === 'overdue' ? 'warning' : 'success',
      width: 100,
      alignment: 'center',
      ariaLabel: `Overdue: ${status === 'overdue' ? String(batchSize).split(' ')[0] : '0'}`,
    },
  ];
