    ...
    switch (timeRange) {
      ...
      case MetricTimeRange.QUARTER:
        ...
        if (item.date.startsWith('Week')) {
          ...
          day = `W${
            new Date(item.date).getMonth() * 4 + Math.floor(new Date(item.date).getDate() / 7) + 1
          }`;
        }
        break;
...
export const StatContainer: React.FC<StatContainerProps> = ({
  ...
  const [rightAverage, setRightAverage] = useState<string | number | undefined>(
    initialRightAverage,
  );
  ...
  useEffect(() => {
    ...
    const fetchMetrics = async () => {
      ...
      try {
        ...
        setRightChartData(
          transformMetricDataToChartData(response.space_utilization_data, timeRange),
        );
      } catch (err) {
        ...
        <CardContent sx={{ p: 1.5 }}>
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, mb: 0.5 }}>
              {icon && <Box sx={{ display: 'flex', alignItems: 'center', height: 20 }}>{icon}</Box>}
              <Typography
                variant="subtitle2"
                fontWeight="normal"
            ...
            <Box sx={{ display: 'flex', alignItems: 'baseline' }}>
              <Typography variant="h4" fontWeight="500" sx={{ lineHeight: 1.1 }}>
                {value}
              </Typography>
              {comparisonValue && (
